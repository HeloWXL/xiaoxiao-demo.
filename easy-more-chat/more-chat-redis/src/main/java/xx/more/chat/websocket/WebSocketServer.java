package xx.more.chat.websocket;
...
public class WebSocketServer {
    ...
    public void onMessage(String message, Session session) {
        ...
        if (StringUtils.isNotBlank(message)) {
            try {
                ...
                redisUtil.publish(ConstantUtils.TOPIC_MSG, jsonObject.toString());
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    public static void sendMessage(String toUser,String message) throws IOException {
        webSocketMap.get(toUser).session.getBasicRemote().sendBinary(ByteBuffer.wrap(message.getBytes("utf-8")));
